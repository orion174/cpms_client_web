import {
    Card,
    CardHeader,
    CardBody,
    Form,
    Container,
    Row,
    Col,
} from "reactstrap";
import { useLocation } from "react-router-dom";
import { useState } from "react";

import Header from "@/view/layout/Headers/Header.tsx";
import SideInfoForm from "@/view/page/setting/user/view/components/SideInfoForm.tsx";
import UserFormButton from "@/view/page/setting/user/form/components/UserFormButton.tsx";
import BasicInfoSection from "@/view/page/setting/user/form/components/BasicInfoSection.tsx";
import CompanyInfoSection from "@/view/page/setting/user/form/components/CompanyInfoSection.tsx";
import EtcInfoSection from "@/view/page/setting/user/form/components/EtcInfoSection.tsx";

import { getInitUser, ReqUserDTO } from "@/definition/user.types.ts";

const UserForm:React.FC = () => {
    const location = useLocation();
    const isEditMode = new URLSearchParams(location.search).has("userId");

    const [ isIdVerified, setIsIdVerified ] = useState<boolean>(false); // 아아디 중복검사 통과여부
    const [ reqUserDTO, setReqUserDTO ] = useState<ReqUserDTO>(getInitUser());

    const handleChange = (field: keyof ReqUserDTO, value: string | number) => {
        setReqUserDTO((prev) => ({
            ...prev,
            [field]: value,
        }));
    };

    return (
        <>
            <Header />

            <Container className={isEditMode ? "mt--5" : "mt--7"} fluid>
                <Row>
                    {/* 오른쪽 기타 정보 표시 영역 */}
                    <Col className="order-xl-2 mb-5 mb-xl-0" xl="4">
                        {isEditMode && (
                            <SideInfoForm />
                        )}
                    </Col>

                    {/* 계정 생성 입력 폼 */}
                    <Col className="order-xl-1" xl={isEditMode ? 8 : 12}>
                        <Card className="bg-secondary shadow">

                            <CardHeader className="bg-white border-0">
                                <Row className="align-items-center">
                                    <Col xs="8">
                                        <h3 className="mb-0">{isEditMode ? '계정 정보 수정' : '계정 생성'}</h3>
                                    </Col>
                                    <Col className="text-right" xs="4">
                                        {/* 폼 버튼 */}
                                        <UserFormButton
                                            reqUserDTO={reqUserDTO}
                                            isEditMode={isEditMode}
                                        />
                                    </Col>
                                </Row>
                            </CardHeader>

                            <CardBody>
                                <Form>
                                    {/* 기본 정보 입력 폼 */}
                                    <BasicInfoSection
                                        reqUserDTO={reqUserDTO}
                                        handleChange={handleChange}
                                        isIdVerified={isIdVerified}
                                        setIsIdVerified={setIsIdVerified}
                                    />

                                    {/* 회사 정보 입력 폼 */}
                                    <CompanyInfoSection
                                        reqUserDTO={reqUserDTO}
                                        handleChange={handleChange}
                                    />

                                    {/* 기타 정보 입력 폼 */}
                                    <EtcInfoSection
                                        reqUserDTO={reqUserDTO}
                                        handleChange={handleChange}
                                    />
                                </Form>
                            </CardBody>

                        </Card>
                    </Col>
                </Row>
            </Container>
        </>
    );
};

export default UserForm;
